        ...
        <?php foreach ($managment as $row) { ?>
          <tr>
            ...
            <td class="text-center">
              <a href="<?= base_url('api/managment/generate/' . $row['id']) ?>" target="_blank" class="btn btn-icon btn-primary btn-sm">
...
<script>
  const KTDatatables = function() {
    ...
    const exportButtons = () => {
      const documentTitle = 'Mantenimientos';
      const columnasAExportar = [0, 1, 2, 3, 4];
      const buttons = new $.fn.dataTable.Buttons(dt_managment, {
        buttons: [{
            extend: 'copyHtml5',
            title: documentTitle,
            exportOptions: {
