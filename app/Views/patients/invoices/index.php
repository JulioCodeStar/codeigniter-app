  ...
  <div class="card-header border-0 pt-6">
    ...
    <div class="card-title">
      ...
      <div class="d-flex align-items-center position-relative my-1">
        ...
        <input type="text" data-kt-invoice-table-filter="search" class="form-control form-control-solid w-250px ps-13" placeholder="Buscar Cotización" />
      </div>
      <!--end::Search-->
    </div>
    <!--begin::Card title-->
    ...
    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_invoice">
      <thead>
        <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
          ...
          <th class="min-w-125px text-center">Encargado</th>
          <th class="min-w-125px text-center">Monto</th>
          <th class="min-w-125px text-center">Fecha</th>
        ...
        <?php foreach ($list as $row) { ?>
          <tr>
            <td class="text-center">
              <span class="badge badge-secondary badge-lg"><?= $row['cod_cotizacion'] ?></span>
            </td>
            <td class="text-center">
              <span class="badge badge-info badge-lg"><?= $row['cod_paciente'] ?></span>
            </td>
            <td class="d-flex flex-column">
              <h5><?= $row['nombres'] . ' ' . $row['apellidos'] ?></h5>
              <p><?= $row['servicio'] . ' - ' . $row['trabajo'] ?></p>
            </td>
            <td class="text-center">
              <?= $row['encargado'] ?>
            </td>
            <td class="text-center">
              <span class="badge badge-light-dark badge-lg">
                <?= ($row['moneda'] == 'PEN' ? 'S/.' : '$') . ' ' . moneda($row['monto_final']) ?>
              </span>
            </td>
            <td class="text-center">
              <?= fecha_dmy($row['fecha_now']) ?>
            </td>
            <td class="text-end">
              ...
              <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                <!--begin::Menu item-->
                <div class="menu-item px-3">
                  <a href="<?= base_url('invoice/show/' . $row['id']) ?>" class="menu-link px-3">
                    Editar
                  </a>
                </div>
                <!--end::Menu item-->
                ...
                <!--begin::Menu item-->
                <div class="menu-item px-3">
                  <a href="#" class="menu-link px-3" data-bs-toggle="modal" data-bs-target="#eliminarModal" data-bs-url="<?= base_url('api/invoice/delete/' . $row['id']) ?>">
                    Eliminar
                  </a>
                </div>
                <!--end::Menu item-->
                ...
                <div class="menu-item px-3">
                  <a href="<?= base_url('invoice/generate/' . $row['id']) ?>" target="_blank" class="menu-link px-3">
                    Pdf
                  </a>
                </div>
              </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content position-absolute">
      ...
      <div class="modal-body">
        <p>¿Deseas Eliminar a esta Cotización del sistema?</p>
      </div>
...
<script>
  const KTDataTablesPatient = function() {
    ...
    const exportButtons = () => {
      const documentTitle = 'Cotizaciones';
    ...
    const handleSearchInvoice = () => {
      const filter = document.querySelector('[data-kt-invoice-table-filter="search"]');
